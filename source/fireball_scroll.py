...
class FireballScroll(Entity):
    ...
    def apply_damage(self, grid_x, grid_y, amount, results):
        ...
        sprites = arcade.get_sprites_at_point((pixel_x, pixel_y), self.game_engine.entities)
        for sprite in sprites:
            if sprite.fighter and not sprite.is_dead:
                results.extend([{"message": f"{sprite.name} was struck by a fireball for {amount} points."}])
